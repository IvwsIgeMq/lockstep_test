...
-- luajit 5.1 模拟5.2功能，
local function  pairs_52 (tab)
	 	local mat = getmetatable(tab)
		if mat and mat.__pairs and type(mat.__pairs) =="function" then
			return mat.__pairs(tab)
		else
			return pairs(tab)
		end
end

local function  ipairs_52 (tab)
	 	local mat = getmetatable(tab)
		if mat and mat.__ipairs and type(mat.__ipairs) =="function" then
			return 	mat.__ipairs(tab)
		else
			return ipairs(tab)
		end
end
...
local function encode_message(CObj, message_type, t)
	local type_ = encode_type_cache[message_type]
	-- G_dump("encode_message",t)
	if type(t) == "table" then -- 去除非表的字段
...
function decode_message_mt.__ipairs(tbl)
	expand(tbl)
	return ipairs(tbl)
end
